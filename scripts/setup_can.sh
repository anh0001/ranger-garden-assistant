...
if ip link show can_base &> /dev/null; then
    echo "Setting up can_base (Ranger base)..."
    sudo ip link set can_base down 2>/dev/null || true
    sudo ip link set can_base type can bitrate 500000
    sudo ip link set can_base up
    echo "✓ can_base is up at 500 kbps"
else
    echo "⚠ can_base not found. Please check CAN adapter connection."
fi
...
ip -details -statistics link show can_base 2>/dev/null || echo "can_base: not available"
