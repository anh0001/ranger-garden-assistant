...
**CRITICAL**: Whenever working with Nav2 parameters, configurations, or troubleshooting navigation issues, ALWAYS consult the official Nav2 documentation:
- Nav2 Tuning Guide: https://docs.nav2.org/tuning/index.html (for all parameter adjustments)
- Nav2 Getting Started: https://docs.nav2.org/getting_started/index.html (for setup and integration)
- Main Nav2 docs: https://navigation.ros.org/ (for concepts and architecture)
...
- The local costmap uses a voxel layer fed by `/livox/lidar`; add an RGB-D layer only if a depth camera is installed (not used with the C2-176).
...
- **Nav2 Documentation**:
  ...
  - Tuning Guide: https://docs.nav2.org/tuning/index.html (essential for parameter tuning)
